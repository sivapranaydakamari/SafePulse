...
class _CirclePageState extends State<CirclePage> {
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Container(
            ...
            child: Column(children: [
              QrImageView(data: code, version: QrVersions.auto, size: 200.0, foregroundColor: Colors.black),
              const SizedBox(height: 20),
              Text(code, style: const TextStyle(
                  color: AppColors.primary, fontSize: 32, fontWeight: FontWeight.bold, letterSpacing: 8)),
            ]),
          ),
          const SizedBox(height: 40),
          ElevatedButton.icon(
            onPressed: () {
              Share.share("Join my SafePulse Circle!\nInvite code: $code\n\nDownload SafePulse for real-time family safety!");
            },
  ...
  void _showAddMembersSheet(String inviteCode) async {
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _AddMembersSheet(contacts: withPhone, inviteCode: inviteCode),
    );
  }
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              if (result['success']) {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text(result['message'] ?? "Failed to join")));
              }
            },
  ...
  void _showScanner() async {
    ...
    if (status.isPermanentlyDenied) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text("Camera permission is required. Enable it in settings.")));
      return;
    }
      ...
      builder: (context) => Container(
        ...
        child: Column(children: [
          ...
          Padding(
            ...
            child: Column(children: [
              ElevatedButton(
                onPressed: () { Navigator.pop(context); _showJoinDialog(); },
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        child: _isLoading
            ...
            : _circles.isEmpty ? _buildEmptyState() : _buildCircleList(),
      ),
    );
  }
  ...
  Widget _buildLiveStatsRow(List members) {
    ...
    for (var m in members) {
      ...
      final battery = int.tryParse((m['batteryLevel'] ?? '100').replaceAll('%', '')) ?? 100;
      if (battery <= 20) lowBatteryCount++;
    }
    return Wrap(spacing: 8, runSpacing: 8, children: [
      _buildStatChip(Icons.people, "${members.length} Members", AppColors.primary),
      if (drivingCount > 0) _buildStatChip(Icons.directions_car, "$drivingCount Driving", Colors.orange),
      if (lowBatteryCount > 0) _buildStatChip(Icons.battery_alert, "$lowBatteryCount Low Battery", Colors.red),
    ]);
  }
  ...
  Widget _buildMemberTile(BuildContext context, {
    required String name, required String status, required Color statusColor,
    required String battery, required bool isDriving, required bool isSelf,
    Color? batteryColor, int currentSpeed = 0, String? profilePic,
    String? circleId, String? circleName, Map<String, dynamic>? memberData,
  }) {
    ...
    return GestureDetector(
        ...
        if (memberData != null && circleId != null && circleName != null) {
          Navigator.push(context, MaterialPageRoute(
            builder: (_) => MemberDetailPage(
              memberData: memberData, circleId: circleId, circleName: circleName,
              isSelf: isSelf,
      ...
      child: Container(
        ...
        child: Row(children: [
          Stack(children: [
            CircleAvatar(
              radius: 28, backgroundColor: AppColors.surface,
                    ...
                    return ListTile(
                      ...
                      trailing: GestureDetector(
                        ...
                        child: AnimatedContainer(
                          ...
                          child: Row(mainAxisSize: MainAxisSize.min, children: [
                            Icon(
                              isInvited ? Icons.check_circle_outline : Icons.send_rounded,
                              color: isInvited ? AppColors.safe : const Color(0xFF25D366),
                              size: 14,
                            ),
